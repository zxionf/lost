...
import com.zyx.kga.lost.MyContactListener;
...
public class MPB extends MyGame {
    ...
    public void render(float p) {        
        ...
        batch.begin();
        batch.draw(bullet, Info.bulletposition[0] * PPM, Info.bulletposition[1] * PPM, 2, 1);
        ...
        if (player1.getPosition().y < -10) {
            ...
            ib.addInfo("[RED][GM][YELLOW]" + Info.playerName + "[WHITE]掉出了世界");
    ...
    public void camupdate(boolean smooth) {
        b2dcam.zoom = Info.cameraheightscale;
        cam.zoom = 1 / PPM * Info.cameraheightscale;
